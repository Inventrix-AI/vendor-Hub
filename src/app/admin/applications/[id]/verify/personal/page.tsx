...
// Personal document types - must match the types stored during registration
// Include legacy types (aadhaar, photo) for backwards compatibility with older registrations
const PERSONAL_DOC_TYPES = ['passport_photo', 'photo', 'aadhaar_card', 'aadhaar', 'voter_id', 'driving_license', 'pan_card']
...
export default function PersonalVerificationPage() {
  ...
  const verifyMutation = useMutation(
    ...
    },
    {
      onSuccess: () => {
        ...
        router.push(`/admin/applications/${applicationId}/verify/business`)
      },
      onError: () => {
  ...
  // Filter personal documents - exact match on document type
  const personalDocs = (application.documents || []).filter((doc: any) =>
    PERSONAL_DOC_TYPES.includes(doc.document_type.toLowerCase())
  )
    ...
    <Layout>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          ...
          {/* Step Progress */}
          <div className="mt-6">
            <p className="text-sm text-gray-600 mb-2">Step 1 of 3: Personal Verification</p>
            <VerificationProgress
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <DetailFieldGroup title="Personal Details">
              <div className="flex items-center mb-4">
                <User className="w-5 h-5 text-gray-400 mr-2" />
                <span className="text-sm font-medium text-gray-500">Applicant Information</span>
              </div>

              <DetailField
                label="Full Name"
                value={application.user_full_name || application.company_name}
              />

              <DetailField
                label="Email"
                value={application.user_email || application.contact_email}
              />

              <DetailField
                label="Mobile Number"
                value={application.user_phone || application.phone}
              />

              {application.age && (
                <DetailField label="Age" value={application.age} />
              )}
            ...
            <DetailFieldGroup title="Identity Information">
              <div className="flex items-center mb-4">
                <CreditCard className="w-5 h-5 text-gray-400 mr-2" />
                <span className="text-sm font-medium text-gray-500">ID Documents</span>
              </div>

              {(() => {
                // Find the actual ID document (not passport_photo)
                const idDocTypes = ['aadhaar_card', 'aadhaar', 'voter_id', 'driving_license', 'pan_card']
                const idDoc = personalDocs.find((d: any) => idDocTypes.includes(d.document_type.toLowerCase()))
                return (
                  <DetailField
                    label="ID Type"
                    value={idDoc?.document_type?.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase()) || 'Not specified'}
                  />
                )
              })()}

              {application.id_number && (
                <DetailField label="ID Number" value={application.id_number} />
              )}
            </DetailFieldGroup>
          </div>
          ...
          <div>
            <DocumentPanel
              ...
              selectedDocumentType={selectedDocType || personalDocs[0]?.document_type}
